...
SoftwareSerial hydroSerial(HYDRO_RX, HYDRO_TX); // RX, TX
...
void setup(void) {
  ...
  pinMode(HYDRO_RX, INPUT);
  pinMode(HYDRO_TX, OUTPUT);
...
void loop(void) {
  ...
  msg = "";
  c = '\x00';
  if (hydroSerial.available()) {
    Serial.println(hydroSerial.read());
  }
  if (Serial.available()) {
    hydroSerial.write(Serial.read());
  }
